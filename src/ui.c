...
void UI_draw_bg(SDL_Renderer* const renderer, const Settings* const settings) {
    ...
    for (uint8_t x = 0; x < settings->tiles.x; x++)
        for (uint8_t y = 0; y < settings->tiles.y; y++) {
            const Rect rect = {
              x * (settings->tile_size + settings->tile_offset) + LEFT_PADDING,
              y * (settings->tile_size + settings->tile_offset) + TOP_PADDING,
              settings->tile_size,
              settings->tile_size
            };
...
void UI_draw_game_over(
    ...
    for (uint8_t i = 1; i <= LOOPS; i++) {
        ...
        Text_draw(&game_over, renderer, assets);
